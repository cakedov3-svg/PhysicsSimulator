...
class Viewer extends SimulationViewer {
	...
	private void initGUI() {
		...
		addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				switch (e.getKeyChar()) {
				case 'j': {
					_originX += 10; //Mover a la izq
					repaint();
					break;
				}
				case 'l': {
					_originX -= 10; //Mover a la der
					repaint();
					break;
				}
				case 'i': {
					_originY += 10; //Mover arriba
					repaint();
					break;
				}
				case 'm': {
					_originY -= 10; //Mover abajo
					repaint();
					break;
				}
				case 'k': {
					_originX = 0;
					_originY = 0; //Centra el origen de nuevo
					repaint();
					break;
				}
				case 'h': {
	...
	private void showHelp(Graphics2D g) {
		...
		g.drawString("g: show next group", 5, 30);
		g.drawString("l: move right, j: move left, i: move up, m: move down: k: reset", 5, 45);
		g.drawString("Scaling ratio: " + _scale, 5, 60);
		g.setColor(Color.BLUE);
		if(_selectedGroup == null) g.drawString("Selected Group: all", 5, 75);
		else g.drawString("Selected Group: " + _selectedGroup, 5, 75);
	}
	...
	private void drawBodies(Graphics2D g) {
		...
		for(Body b : _bodies) {
			if(isVisible(b)) {
				...
				int posX = _centerX + (int) (pos.getX()/_scale);
				int posY = _centerY - (int) (pos.getY()/_scale);
				g.fillOval(posX - 5, posY - 5, 10, 10);
				g.setColor(Color.BLACK);
				g.drawString(b.getId(), posX - 5, posY - 10);
				if(_showVectors) {
					//Flecha velocidad
					Vector2D v = b.getVelocity().direction().scale(20);
					int vX = posX + (int) (v.getX()), vY = posY - (int) (v.getY());
					this.drawLineWithArrow(g, posX, posY, vX, vY, 3, 3, Color.GREEN, Color.GREEN);
					
					//Flecha fuerza
					Vector2D f = b.getForce().direction().scale(20);
					int fX = posX + (int) (f.getX()), fY = posY - (int) (f.getY());
					this.drawLineWithArrow(g, posX , posY, fX, fY, 3, 3, Color.RED, Color.RED);
				}
			}
		}
	...
	private boolean isVisible(Body b) {
		...
		else return b.getgId().equals(_selectedGroup);
	}
