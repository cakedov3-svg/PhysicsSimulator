...
class GroupsTableModel extends AbstractTableModel implements SimulatorObserver {
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		if(rowIndex < 0 || rowIndex >= _groups.size() || columnIndex < 0 || columnIndex >= _header.length)
			throw new IllegalArgumentException("Posicion ilegal de la tabla seleccionada");
		String result = "";
		
		switch(columnIndex) {
		case 0:
			result = _groups.get(rowIndex).getId();
			break;
		case 1:
			result = _groups.get(rowIndex).getForceLawsInfo();
			break;
		case 2:
